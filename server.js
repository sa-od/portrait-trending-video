  ...
  try {
    ...
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "You are a helpful assistant that generates video titles. Always respond with valid JSON containing an array of titles."
        },
        {
          role: "user",
          content: prompt
...
app.post('/api/generate-video', async (req, res) => {
  try {
    ...
    if (!videoFilename || !titles || !Array.isArray(titles) || titles.length !== 3) {
